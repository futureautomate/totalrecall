...
// stop this: an env guard (SESSIONTRACK_DIGESTER=1, set by claude-runner.ts
...
// the real dbPath() (~/.sessiontrack/index.db) with no test-injection seam —
...
describe("hook-run guards (live-incident regression)", () => {
  it("env guard: exits 0 immediately when SESSIONTRACK_DIGESTER=1, before touching stdin/DB", async () => {
    const { code, ms } = await runHookRun({ env: { SESSIONTRACK_DIGESTER: "1" } });
    expect(code).toBe(0);
